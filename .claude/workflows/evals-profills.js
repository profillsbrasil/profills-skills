...
const HOJE = (args && args.hoje) || '2026-08-24' // data congelada: fixtures e expectations usam datas literais
...
const runPrompt = (e, cfg) => {
  const skillPath = cfg === 'atual' ? REPO + '/skills/' + e.skill : plano.snapshot + '/' + e.skill
  ...
  return [
    ...
    '- Diretório de trabalho: ' + runDir + '/repo (repo git com linkedin-data/ na raiz). Caminhos absolutos a partir dele. NUNCA leia nem escreva em ' + REPO + '/linkedin-data, em ~/Profills LinkedIn, em ~/.config nem em ~/.claude; o que a skill mandar gravar em ~/.config vai em ' + runDir + '/repo/config/ (diga isso no transcript).',
    '- A data de hoje neste caso é ' + HOJE + ': use-a em toda data que gravar e passe --hoje ' + HOJE + ' aos scripts da skill que aceitam a flag.',
    '- O disco do sandbox já reflete o estado inicial do caso dentro de repo/. O "(Contexto: ...)" do pedido vence para tudo o que está fora do sandbox (~/.claude, ~/.agents, comandos claude/npx, tools) e para o que o usuário responde. Não invente estado além disso.',
...
const resultados = await pipeline(
  plano.evals,
  e => parallel(['atual', 'baseline'].map(cfg => () =>
    agent(runPrompt(e, cfg), { label: 'run:' + e.skill.replace('profills-', '') + '#' + e.id + (cfg === 'atual' ? ':atual' : ':base'), phase: 'Rodar', schema: RUN, model: 'sonnet' })
  )).then(rs => ({ e, runs: rs })),
