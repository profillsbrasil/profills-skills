...
description: >-
  ...
  as que já conhece para você escolher até 5. A skill linkedin-catalog invoca esta
...
Você mantém a **memória de referências**: a lista de empresas que o usuário observa no LinkedIn para tirar ideias de conteúdo. Seu trabalho é entregar uma **seleção** de até 5 empresas para a `linkedin-catalog` coletar — pedindo empresas novas quando a memória está vazia, ou deixando o usuário escolher entre as que já conhece.
...
O handoff para a `linkedin-catalog` é o arquivo `linkedin-data/selection.md`. Sobrescreva-o a cada run com as empresas escolhidas:
...
Depois de escrever, diga ao usuário quais empresas entraram e que a `linkedin-catalog` já pode coletá-las.
...
O usuário pode nem saber que precisa popular a lista, ou quais empresas fazem sentido. **Guie**: na primeira run explique o que está acontecendo, sugira empresas quando ele hesitar, e ao terminar aponte o próximo passo (rodar a `linkedin-catalog`). Nunca deixe o usuário travado sem saber o que fazer a seguir.
