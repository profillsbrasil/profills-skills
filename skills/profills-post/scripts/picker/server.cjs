...
<body><!-- BRANDING --><h1>Picker de posts</h1>
<p>Esperando os rascunhos no formato do feed...</p></body></html>`);
}
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Abrindo o picker de posts</title></head>
<body>
<script>
...
const frameTemplate = fs.readFileSync(path.join(__dirname, 'picker-frame.html'), 'utf-8');
...
function brandMarkup() {
  return '<div class="brand"><span class="brand-copy">Profills</span></div>';
}
...
function postsJsonToFragment(raw) {
  let data;
  try {
    data = JSON.parse(raw);
  } catch (e) {
    data = {
      ...
      subtitulo: 'Essa revisão não entrou. Escreva um arquivo novo com as opções.',
      empresa: { nome: 'Profills', iniciais: 'P' },
      opcoes: []
    };
  }
  return 'const DADOS = ' + JSON.stringify(data) + ';\n';
}

function screenToHtml(filePath) {
  const raw = fs.readFileSync(filePath, 'utf-8');
  if (filePath.endsWith('.json')) return wrapInFrame(postsJsonToFragment(raw));
  return isFullDocument(raw) ? raw : wrapInFrame(raw);
}

function getNewestScreen() {
  const files = fs.readdirSync(CONTENT_DIR)
    .filter(f => !f.startsWith('.') && isScreenFile(f))
    .map(f => {
...
function startServer() {
  ...
  const knownFiles = new Set(
    fs.readdirSync(CONTENT_DIR).filter(f => !f.startsWith('.') && isScreenFile(f))
  );
  ...
  const watcher = fs.watch(CONTENT_DIR, (eventType, filename) => {
    if (!filename || filename.startsWith('.') || !isScreenFile(filename)) return;
  ...
  function shutdown(reason) {
    ...
    function finalize() {
      ...
      const idFile = path.join(STATE_DIR, 'server-instance-id');
  ...
  function onListen() {
    ...
    const info = JSON.stringify({
      ...
      url_host: URL_HOST,
      url: 'http://' + urlHostForHttp(URL_HOST) + ':' + PORT + '/',
